...
const EditPressEditor = dynamic(() => import("./EditPressEditor"), {
  ssr: false,
});
...
export default function EditPress() {
  ...
  const [preview, setPreview] = useState(
    pressData.bannerImage || "/banner.png"
  );
  ...
  const updateTitleContext = useRef(
    debounce((value) => {
      setPressData((prev) => ({ ...prev, title: value }));
    }, 300)
  ).current;
  ...
  return (
    <div className='bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-md transition-colors duration-300'>
      ...
      <EditPressEditor preview={preview} />

      <div className='mt-6 flex justify-end'>
        <Link href='/dashboard/press/edit-preview-press'>
          <Button
            type='submit'
