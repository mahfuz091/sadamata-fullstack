...
import { pressCreate } from "@/app/actions/press/press.actions";
import { PressContext } from "@/context/PressContext";

const SeePress = ({ userId }) => {
  const router = useRouter();
  const { pressData, setPressData } = useContext(PressContext);
  console.log("pressData: Preview", pressData);

  if (!pressData) return <p>No Press data available</p>;
  ...
  const handlePublish = async (e) => {
    ...
    formData.append("postSlug", pressData.postSlug);
    formData.append("bannerAltText", pressData.bannerAltText);
    formData.append("metaTitle", pressData.metaTitle);
    formData.append("metaDescription", pressData.metaDescription);
    formData.append("canonicalUrl", pressData.canonicalUrl);
    ...
    startTransition(async () => {
      try {
        const response = await pressCreate(null, formData);
        console.log("response:", response);

        if (response?.success) {
          toast.success(response.msg);
          router.push("/dashboard/press"); // ✅ force redirect
          router.refresh("/dashboard/press");
          setPressData({
            title: "",
            shortDesc: "",
            content: null,
          ...
          toast.error(response?.msg || "Failed to publish Press");
        }
      } catch (err) {
        console.error("publish error:", err);
        toast.error("Server error while publishing press");
      }
    });
  };
  return (
    ...
    >
      ...
      <div className='flex justify-between items-center mb-6'>
        ...
        <Link href='/dashboard/press/add-press'>
