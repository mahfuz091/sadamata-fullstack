...
import { updateUserInfo, updateUserProfile, updateUserProfileImageFile } from '@/app/actions/userAddressActions';
...
useEffect(() => {
  ...
  if (user?.userprofile?.country) {
    const matched = countries.find(
      (c) => c.name === user.userprofile.country
    );
  ...
  const [formData, setFormData] = useState({
    firstName: user.userprofile?.firstName || user.name || user.addresses[0].name || '',
    lastName: user.userprofile?.lastName || user.addresses[0].name || '',
    email: user.email || user.addresses[0].email || '',
    dateOfBirth: user.userprofile?.dateOfBirth ? formatDateForInput(user.userprofile.dateOfBirth) : '',
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const updated = await updateUserProfileImageFile(user.addresses[0].id, selectedFile);
              ...
              <div className="user-profile__group">
                <div className="user-profile__group__item">
                  <label htmlFor="fname">First Name</label>
                    ...
                    <input type="text" name="firstName" id="fname" value={formData.firstName} onChange={handleChange} readOnly={!editMode}  />
                  
                </div>
                <div className="user-profile__group__item">
                  <label htmlFor="lname">Last Name</label>
                  
                    <input type="text" name="lastName" id="lname" value={formData.lastName} onChange={handleChange} readOnly={!editMode}  />
