...
export async function getTodayMerchantSalesReportFromOrders(merchantId) {
  ...
  const paidOrders = await prisma.order.findMany({
    ...
    select: {
      id: true,
      items: {
        select: { id: true, productId: true, unitPrice: true, quantity: true },
      },
    },
  });
  ...
  const cancelledOrders = await prisma.order.findMany({
    where: { status: "CANCELLED", cancelledAt: { gte: startUTC, lt: endUTC } },
  ...
  for (const order of paidOrders) {
    ...
    for (const item of order.items) {
      ...
      const brandPct =
        p.brandCommissionPct ?? (p.brandId ? p.Brand?.defaultBrandPct ?? 0 : 0);
      const merchPct =
