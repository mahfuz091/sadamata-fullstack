...
export default function AddDesignC() {
  ...
  const initializeCanvas = (ref, imageSrc, setCanvas, designImage) => {
    // Ensure ref.current is not null and get the current canvas instance
    if (ref.current) {
      // Retrieve the previous canvas from the state (if any)
      const existingCanvas = ref.current._fabricCanvas;
    ...
    const fabricCanvas = new Canvas(ref.current, {
      width: 400,
      height: 400,
      selection: false,
    });
    ...
    mockupImg.src = imageSrc;

    mockupImg.onload = () => {
      fabricCanvas.clear(); // Clear any existing content

      // Add the mockup image to the canvas
      ...
      fabricCanvas.renderAll(); // Render the canvas to show the mockup

      if (designImage) {
        // If design image is present, add it to the canvas
        addDesignToCanvas(fabricCanvas);
      }
    };

    setCanvas(fabricCanvas); // Update the canvas state
  };
  ...
  useEffect(() => {
    // Initialize the canvas based on the current view
    ...
    initializeCanvas(canvasRef, mockupImage, setCanvas, designImage);
  }, [designImage]); // Reinitialize when the view or design image changes
...
<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 50 50">
  ...
  //     designImage
  //   );
  //   initializeCanvas(canvasTwoRef, mockupImage, setCanvasTwo, designImage);
  // }, [designImage]);
  const handleColorChange = (color) => {
    setSelectedColor(color); // Update selected color
    // initializeCanvas(); // Reinitialize the canvas with the new mockup
  };
