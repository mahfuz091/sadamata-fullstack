...
export default function AddDesignFitNew2({
  ...
  // Multi-select (arrays) + hover for deterministic preview
  ...
  useEffect(() => {
    ...
    // re-init only if grid length changes (new cards)
  }, [allMockup.length]);
  ...
  // Defaults when product changes: seed arrays
  useEffect(() => {
  ...
  // ---------- Renders (reuse same canvas instances) ----------
  // Front preview render
  useEffect(() => {
  ...
  // Grid renders (no re-init; just redraw on changes)
  useEffect(() => {
  ...
  // Normalize rect from preview canvas to percentages
  ...
  // Compose a side (FRONT/BACK) to Blob at base image resolution
  ...
  // SINGLE product FormData for createProduct; respects checkbox selections
                        ...
                        <div className="product-preview-panel__color-chooser">
                          ...
                          {/* <div className="color-options d-flex gap-2 flex-wrap">
                            {(activeColors ?? []).map((color) => {
                              const checked =
                                (selectedColor[activeProductIndex] ?? []).includes(
                                  color
                                );
                              return (
